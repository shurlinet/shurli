...
You'll immediately see the dashboard with 56 panels (45 visualizations + 11 row headers) across 11 sections.
...
| `shurli_vault_unseal_total` | Counter | result | Remote unseal attempts (success/failure/denied/blocked/locked_out/error) |
| `shurli_vault_unseal_locked_peers` | Gauge | - | Peers currently in lockout or permanently blocked |
| `shurli_deposit_operations_total` | Counter | operation | Invite deposit lifecycle (create/revoke/modify) |
| `shurli_deposit_pending` | Gauge | - | Pending unconsumed invite deposits |
...
| `shurli_admin_request_duration_seconds` | Histogram | endpoint | Admin socket request latency |
