...
**Reference**: `pkg/sdk/peerrelay.go`, `internal/config/config.go` (PeerRelayConfig)
...
**Decision**: `RelayDiscovery` in `pkg/sdk/relaydiscovery.go`. Three components:
...
**Reference**: `pkg/sdk/relaydiscovery.go`, `pkg/sdk/relaydiscovery_test.go`
...
**Decision**: `RelayHealth` in `pkg/sdk/relayhealth.go`. Per-relay `RelayHealthScore` tracks:
...
**Reference**: `pkg/sdk/relayhealth.go`, `pkg/sdk/metrics.go`
...
**Decision**: `BandwidthTracker` in `pkg/sdk/bandwidth.go`. Wraps libp2p's `metrics.BandwidthCounter` (wired via `libp2p.BandwidthReporter()`). Exposes:
...
**Reference**: `pkg/sdk/bandwidth.go`, `pkg/sdk/bandwidth_test.go`, `internal/daemon/handlers.go`
...
2. **DNS seeds** (`_dnsaddr.<domain>` TXT records) - default: `seeds.shurli.io`. Uses the dnsaddr multiaddr convention from IPFS. `ResolveDNSSeeds()` in `pkg/sdk/dnsseed.go` queries TXT records with 10s timeout. Records format: `dnsaddr=/ip4/203.0.113.50/tcp/7777/p2p/12D3KooW...`. Merges addresses for same peer ID. DNS failures are logged but not fatal.
...
**Reference**: `pkg/sdk/dnsseed.go`, `pkg/sdk/dnsseed_test.go`, `cmd/shurli/seeds.go`, `cmd/shurli/cmd_seed_helpers.go`
