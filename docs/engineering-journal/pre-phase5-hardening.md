...
**Reference**: `cmd/shurli/serve_common.go`, `pkg/sdk/stunprober.go`, `pkg/sdk/reachability.go`
...
**Reference**: `pkg/sdk/netmonitor.go`, `cmd/shurli/serve_common.go`
...
Both platforms use the pure-Go `sd_notify` implementation in `internal/watchdog/` (no CGo, no-op on non-systemd). The watchdog checks health every 30s, sends `WATCHDOG=1` to systemd on success. `WatchdogSec=90` (3x interval) triggers restart if health checks stop. On macOS, launchd's `KeepAlive` provides equivalent crash recovery.
