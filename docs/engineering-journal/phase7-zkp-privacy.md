...
**Decision**: 9 new Prometheus metrics added to `pkg/sdk/metrics.go`:
...
**Modified files**:
- `pkg/sdk/metrics.go` - 9 new ZKP metrics
...
**Reference**: `pkg/sdk/netintel.go` (NodeAnnouncement struct)
...
**Modified files**:
- `pkg/sdk/netintel.go` - `AnonymousMode` + `ZKPProof` fields on `NodeAnnouncement`
- `pkg/sdk/metrics.go` - 5 new range proof / anonymous metrics
