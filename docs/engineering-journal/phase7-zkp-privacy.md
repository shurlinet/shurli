...
**Reference**: `internal/zkp/poseidon2.go`, `internal/zkp/poseidon2_test.go`
...
**Reference**: `internal/zkp/merkle.go`, `internal/zkp/merkle_test.go`
...
**Reference**: `internal/zkp/membership.go`, `internal/zkp/membership_test.go`
...
**Reference**: `internal/zkp/srs.go`, `internal/zkp/keys.go`, `internal/zkp/prover.go`, `internal/zkp/verifier.go`

---

### ADR-N06: Binary Size Analysis

**Context**: Binary size needed a detailed breakdown to justify each component.
...
| File | Lines | Purpose |
|------|-------|---------|
| `internal/zkp/errors.go` | 14 | Sentinel errors |
| `internal/zkp/poseidon2.go` | 72 | Native + circuit Poseidon2 hash wrappers |
| `internal/zkp/poseidon2_test.go` | 277 | 13 tests + 2 benchmarks |
| `internal/zkp/merkle.go` | 217 | Tree builder, proof generation, verification |
| `internal/zkp/merkle_test.go` | 439 | 13 tests + 4 benchmarks |
| `internal/zkp/membership.go` | 90 | PLONK membership circuit |
| `internal/zkp/membership_test.go` | 261 | 6 tests + 1 benchmark |
| `internal/zkp/srs.go` | 60 | KZG SRS generation with caching |
| `internal/zkp/keys.go` | 91 | Proving/verifying key serialization |
| `internal/zkp/prover.go` | 169 | High-level prover |
| `internal/zkp/verifier.go` | 83 | High-level verifier |
| `internal/zkp/prover_test.go` | 155 | 5 end-to-end tests |
| `docs/images/binary-size-breakdown.svg` | 1 | Pie chart of binary composition |

**Total**: 13 new files, ~1,928 lines of code + tests. 1 modified file (`internal/config/config.go`).
...
**Reference**: `internal/relay/zkp_auth.go`
...
**Decision**: `ChallengeStore` in `internal/zkp/challenge.go`. Nonces are `uint64` from `crypto/rand`. The store tracks pending nonces in a mutex-protected map. `Consume()` atomically removes the nonce regardless of outcome (expired or valid). `CleanExpired()` runs periodically to remove stale entries.
...
**Reference**: `internal/zkp/challenge.go`, `internal/zkp/challenge_test.go`
...
**Reference**: `internal/relay/admin.go` (handlers), `internal/relay/admin_client.go` (client methods)
...
**Decision**: 9 new Prometheus metrics added to `pkg/p2pnet/metrics.go`:
...
**Reference**: `pkg/p2pnet/metrics.go`
...
| File | Lines | Purpose |
|------|-------|---------|
| `internal/zkp/challenge.go` | 105 | Challenge nonce store (issue, consume, clean, pending) |
| `internal/zkp/challenge_test.go` | 130 | 7 tests: replay, expiry, uniqueness, cleanup |
| `internal/relay/zkp_auth.go` | 270 | Relay ZKP handler + wire encoding/decoding helpers |
| `internal/relay/zkp_auth_test.go` | 140 | 8 wire protocol tests |
| `internal/relay/zkp_client.go` | 100 | Client-side ZKP auth (stream-based proof generation) |

**Modified files**:
- `pkg/p2pnet/metrics.go` - 9 new ZKP metrics
- `internal/relay/admin.go` - 2 new endpoints, `SetZKPAuth`, `ZKPTreeInfoResponse`
- `internal/relay/admin_client.go` - `ZKPTreeRebuild`, `ZKPTreeInfo` methods
- `cmd/shurli/cmd_relay_serve.go` - ZKP handler init, stream registration, metrics wiring
...
**Reference**: `internal/reputation/score.go`
...
**Reference**: `internal/zkp/range_proof.go` (27,004 constraints, 520-byte proofs)
...
**Reference**: `pkg/p2pnet/netintel.go` (NodeAnnouncement struct)
...
**Reference**: `internal/zkp/rln_seam.go`
...
| File | Lines | Purpose |
|------|-------|---------|
| `internal/reputation/score.go` | 112 | Deterministic ComputeScore (0-100, 4 components) |
| `internal/reputation/score_test.go` | 182 | 14 tests: edge cases, component isolation, composite |
| `internal/zkp/range_proof.go` | 208 | RangeProofCircuit + RangeProver + RangeVerifier |
| `internal/zkp/range_proof_test.go` | 290 | 11 tests: circuit + 2 end-to-end PLONK |
| `internal/zkp/rln_seam.go` | 52 | RLN types + interface (extension point) |

**Modified files**:
- `pkg/p2pnet/netintel.go` - `AnonymousMode` + `ZKPProof` fields on `NodeAnnouncement`
- `pkg/p2pnet/metrics.go` - 5 new range proof / anonymous metrics
...
**Decision**: Option 3, using BIP39 seed phrases. This approach draws from cryptocurrency conventions (BIP39 is the standard for Bitcoin/Ethereum wallet recovery phrases).
...
**Reference**: `internal/zkp/bip39.go`, `internal/zkp/srs.go` (`SetupKeysFromSeed`), `cmd/shurli/cmd_relay_zkp.go`

---

### ADR-N17: ProvingKey/VerifyingKey Naming Convention

**Context**: Code review flagged a naming collision: "PK" looks like "Private Key" to anyone from a crypto/blockchain background. In PLONK, PK = Proving Key (PUBLIC, ~2 MB) and VK = Verifying Key (PUBLIC, ~34 KB). Both are safe to share freely. The abbreviation collision could lead to dangerous misunderstandings - someone might think the proving key is secret and refuse to distribute it, breaking the ZKP auth flow.
...
**Reference**: `internal/zkp/keys.go`, `internal/zkp/prover.go`, `internal/zkp/verifier.go`, `internal/zkp/range_proof.go`, `internal/zkp/srs.go`
...
**Reference**: `internal/relay/admin.go` (handlers), `internal/relay/admin_client.go` (client methods), `internal/zkp/keys.go` (`ProvingKeyPath`, `VerifyingKeyPath`)
...
| File | Lines | Purpose |
|------|-------|---------|
| `internal/zkp/bip39.go` | 165 | Pure-stdlib BIP39: generate, validate, seed derivation, file I/O |
| `internal/zkp/bip39_wordlist.go` | 2054 | Standard BIP39 English wordlist (2048 words) |
| `internal/zkp/bip39_test.go` | ~150 | 11 tests: generation, validation, checksum, known vectors |
| `internal/zkp/seed_test.go` | ~160 | 3 tests: determinism, different seeds, prove/verify round-trip |

**Modified files**:
- `internal/zkp/srs.go` - Added `SetupKeysFromSeed(keysDir, mnemonic)` using gnark's `WithToxicSeed`
- `internal/zkp/keys.go` - Added `ProvingKeyPath()` and `VerifyingKeyPath()` accessor functions. Renamed file constants and all function parameters from `pk`/`vk` to `provingKey`/`verifyingKey`.
- `internal/zkp/prover.go` - Struct field `pk` -> `provingKey`, all references updated
- `internal/zkp/verifier.go` - Struct field `vk` -> `verifyingKey`, all references updated
- `internal/zkp/range_proof.go` - `RangeProver.pk` -> `provingKey`, `RangeVerifier.vk` -> `verifyingKey`
- `internal/relay/admin.go` - Added `GET /v1/zkp/proving-key` and `GET /v1/zkp/verifying-key` endpoints
- `internal/relay/admin_client.go` - Added `ZKPProvingKey()` and `ZKPVerifyingKey()` client methods
- `internal/relay/zkp_auth.go` - Added `keysDir` field and `KeysDir()` accessor
- `cmd/shurli/cmd_relay_zkp.go` - Added `runRelayZKPSetup` (zkp-setup subcommand), `--seed` flag on zkp-test
- `cmd/shurli/cmd_relay.go` - Added `zkp-setup` case to relay command router
