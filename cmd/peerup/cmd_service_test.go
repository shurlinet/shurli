...
// because unit tests never see the actual config that users get from peerup init.
...
// actually sees after running "peerup init".
func writeRealTemplateConfig(t *testing.T) string {
	t.Helper()
	dir := t.TempDir()

	// Use the real template (the same one peerup init generates)
...
// with a private DHT namespace (from peerup join with a namespace invite).
func TestUserJourneyWithNetworkNamespace(t *testing.T) {
	dir := t.TempDir()

	// Use template with network namespace (as generated by peerup join)
	relayAddr := "/ip4/1.2.3.4/tcp/7777/p2p/12D3KooWDpJ7As7BWAwRMfu1VU2WCqNjvq387JEYKDBj4kx6nXTN"
	configContent := nodeConfigTemplate(relayAddr, "peerup join", "my-private-network")
