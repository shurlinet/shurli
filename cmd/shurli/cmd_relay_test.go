...
names: {}
`
	if err := os.WriteFile(filepath.Join(dir, "shurli.yaml"), []byte(cfg), 0600); err != nil {
		t.Fatalf("write config: %v", err)
	}
	return filepath.Join(dir, "shurli.yaml")
}
...
func TestDoRelayList(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			args: func(t *testing.T) []string {
				return []string{"--config", "/tmp/nonexistent-test-dir-shurli/shurli.yaml"}
			},
			wantErr:    true,
...
func TestResolveAuthKeysPathErr_MissingConfig(t *testing.T) {
	_, err := resolveAuthKeysPathErr("", "/tmp/nonexistent-shurli-config/shurli.yaml")
	if err == nil {
...
func TestResolveConfigFileErr(t *testing.T) {
	...
	t.Run("missing config", func(t *testing.T) {
		_, _, err := resolveConfigFileErr("/tmp/nonexistent-test-dir-shurli/shurli.yaml")
		if err == nil {
			t.Fatal("expected error for missing config")
		}
		if !strings.Contains(err.Error(), "config error") {
