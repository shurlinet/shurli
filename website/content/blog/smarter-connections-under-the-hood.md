...
- **QUIC-preferred transport ordering**: QUIC first (0-1 RTT, native multiplexing), TCP second (universal fallback), WebSocket third (anti-censorship). First transport to succeed wins
- **Identify UserAgent**: Every peer sends `shurli/0.1.0` (or `relay-server/0.1.0`) in the libp2p Identify exchange. `shurli daemon peers` filters by this, showing only your network members by default
