...
Run `shurli daemon` as a managed system service that starts on boot, restarts on crash, and integrates with your platform's logging system.

| Platform | Init system | Service file |
|----------|-------------|--------------|
| Linux | systemd | `shurli-daemon.service` |
| macOS | launchd | `com.shurli.daemon.plist` |
...
The daemon includes a built-in watchdog that is pure Go with no CGo dependency.
