---
title: "Phase 9 - File Transfer Architecture"
weight: 23
description: "FastCDC chunking, BLAKE3 Merkle, zstd compression, Reed-Solomon erasure, RaptorQ multi-source, parallel streams, receive permissions."
---
...
Write our own FastCDC in `https://github.com/shurlinet/shurli/blob/main/plugins/filetransfer/chunker.go` (180 lines). Single-pass streaming: each byte is hashed with BLAKE3 as the chunk boundary is found, so the chunk hash is available the moment the boundary is detected. No second pass.
...
**Reference**: `https://github.com/shurlinet/shurli/blob/main/plugins/filetransfer/chunker.go`
...
**Reference**: `https://github.com/shurlinet/shurli/blob/main/pkg/sdk/merkle.go`, `https://github.com/shurlinet/shurli/blob/main/plugins/filetransfer/chunker.go`
...
**Reference**: `https://github.com/shurlinet/shurli/blob/main/plugins/filetransfer/compress.go` (41 lines), `https://github.com/shurlinet/shurli/blob/main/plugins/filetransfer/transfer.go` (maxDecompressRatio)
...
**Reference**: `https://github.com/shurlinet/shurli/blob/main/plugins/filetransfer/transfer_erasure.go` (384 lines)
...
Wire protocol: `/shurli/file-multi-peer/1.0.0` (`https://github.com/shurlinet/shurli/blob/main/plugins/filetransfer/transfer_multipeer.go`, 874 lines). Requesting peer sends a manifest to each source, each source encodes independently and streams symbols back. The receiver collects symbols from all sources and decodes when it has enough.
...
**Reference**: `https://github.com/shurlinet/shurli/blob/main/plugins/filetransfer/transfer_raptorq.go` (105 lines), `https://github.com/shurlinet/shurli/blob/main/plugins/filetransfer/transfer_multipeer.go`
...
Parallel chunk transfer with transport-aware defaults (`https://github.com/shurlinet/shurli/blob/main/plugins/filetransfer/transfer_parallel.go`, 592 lines):
...
**Reference**: `https://github.com/shurlinet/shurli/blob/main/plugins/filetransfer/transfer_parallel.go`
...
**Reference**: `https://github.com/shurlinet/shurli/blob/main/plugins/filetransfer/transfer.go` (ReceiveMode constants)
...
Implementation: `transferRateLimiter` struct in `https://github.com/shurlinet/shurli/blob/main/plugins/filetransfer/transfer.go`. Per-peer counters with periodic cleanup of stale entries.
...
**Reference**: `https://github.com/shurlinet/shurli/blob/main/plugins/filetransfer/transfer.go` (transferRateLimiter)
